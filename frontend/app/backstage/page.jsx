    ...
    <Modal title={detail.data ? detail.data.name : 'Cohort details'} onClose={onClose}>
      ...
      {detail.data && (
        <div className="space-y-6">
          <div className="sticky top-0 z-10 bg-white -mx-6 -mt-6 px-6 pt-5 pb-4 mb-1 border-b border-slate-100 flex items-center justify-between gap-4">
            <div className="min-w-0">
              <p className="text-slate-900 font-bold text-base leading-tight truncate">{detail.data.name}</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-lg px-4 py-3">
              <p className="text-slate-400 text-[10px] uppercase tracking-widest font-bold mb-1">Tutor</p>
              <p className="text-slate-800 font-semibold text-sm">{detail.data.tutor_name || 'Unassigned'}</p>
            </div>
            <div className="bg-slate-50 rounded-lg px-4 py-3">
            ...
            <div className="bg-slate-50 rounded-lg px-4 py-3">
              <p className="text-slate-400 text-[10px] uppercase tracking-widest font-bold mb-1">Starts</p>
              <p className="text-slate-800 font-semibold text-sm">{formatDate(detail.data.start_date) || '—'}</p>
            </div>
            <div className="bg-slate-50 rounded-lg px-4 py-3">
              <p className="text-slate-400 text-[10px] uppercase tracking-widest font-bold mb-1">Ends</p>
              <p className="text-slate-800 font-semibold text-sm">{formatDate(detail.data.end_date) || '—'}</p>
            </div>
          </div>

          <div>
            <p className="text-slate-900 font-bold text-sm mb-3">Students</p>
            <div className="grid grid-cols-4 gap-2 mb-3">
              {['active', 'inactive', 'expelled', 'withdrawn'].map((key) => (
                <button
                  key={key}
                  onClick={() => setStudentFilter(key)}
                  className={`text-center border rounded-lg py-2.5 transition ${studentFilter === key ? 'border-[#0057E7] bg-blue-50/50 shadow-sm' : 'border-slate-100 hover:border-slate-200'}`}
                >
            ...
            <div className="space-y-1.5 max-h-56 overflow-y-auto rounded-lg border border-slate-100 p-1.5">
              {(detail.data.students || [])
                ...
                .map((s) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3 py-2 rounded-md hover:bg-slate-50 transition text-left"
                  >
                    <div className="min-w-0">
                      <p className="text-slate-800 text-sm font-medium truncate">{s.student_name}</p>
                      <p className="text-slate-400 text-xs truncate">{s.student_email}</p>
                    </div>
              ...
              {(detail.data.students || []).filter((s) => s.status === studentFilter).length === 0 && (
                <p className="text-slate-400 text-xs text-center py-4">No {studentFilter} students.</p>
              )}
            </div>
          ...
          <div className="pt-5 border-t border-slate-100">
            <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
              <p className="text-slate-900 font-bold text-sm">Classes</p>
              <div className="flex items-center gap-1">
                {[
                  ...
                  <button
                    ...
                    className={`text-[11px] font-semibold px-3 py-1.5 rounded-full border transition ${sessionTab === t.key
                      ? 'border-[#0057E7] bg-[#0057E7] text-white shadow-sm'
                      : 'border-slate-200 text-slate-500 hover:border-slate-300 hover:text-slate-700'
                      }`}
                  >
              ...
              <div className="bg-slate-50 rounded-lg p-4">
                <div className="flex items-center justify-between gap-2 mb-2">
                  ...
                  <Pill color={detail.data.today.attendance_taken ? 'emerald' : 'amber'}>
                    {detail.data.today.attendance_taken ? 'Attendance taken' : 'Not marked yet'}
                  </Pill>
                </div>
                ...
                {detail.data.today.session.title && (
                  <p className="text-slate-900 text-sm font-semibold mb-1">{detail.data.today.session.title}</p>
                )}
                ...
                {detail.data.today.roster && (
                  <div className="space-y-1.5 max-h-64 overflow-y-auto mt-2 border-t border-slate-200 pt-3">
                    ...
                    {detail.data.today.roster.map((r) => (
                      <div key={r.application_id} className="flex items-center justify-between px-3 py-2 bg-white rounded-md border border-slate-100">
                        <div className="min-w-0">
                          <p className="text-slate-800 text-sm font-medium truncate">{r.student_name}</p>
